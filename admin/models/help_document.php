...
class QuestionsanswersModelHelp_document extends JModelAdmin
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_questionsanswers.help_document';
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		// Get the form.
		$form = $this->loadForm('com_questionsanswers.help_document', 'help_document', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('help_document.edit.state', 'com_questionsanswers.help_document.' . (int) $id))
			|| ($id == 0 && !$user->authorise('help_document.edit.state', 'com_questionsanswers')))
		{
		...
		if (!$user->authorise('core.edit.created_by', 'com_questionsanswers'))
		{
			// Disable fields for display.
		...
		if (!$user->authorise('core.edit.created', 'com_questionsanswers'))
		{
			// Disable fields for display.
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('help_document.delete', 'com_questionsanswers.help_document.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		{
			// The record has been set. Check the record permissions.
			$permission = $user->authorise('help_document.edit.state', 'com_questionsanswers.help_document.' . (int) $recordId);
		...
		return $user->authorise('help_document.edit.state', 'com_questionsanswers');
	}
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return $user->authorise('help_document.edit', 'com_questionsanswers.help_document.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or $user->authorise('help_document.edit',  'com_questionsanswers');
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__questionsanswers_help_document'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_questionsanswers.edit.help_document.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// run the perprocess of the data
			$this->preprocessData('com_questionsanswers.help_document', $data);
		}
	...
	public function validate($form, $data, $group = null)
	{
		...
		if (isset($data['not_required']) && QuestionsanswersHelper::checkString($data['not_required']))
		{
			...
			foreach ($requiredFields as $requiredField)
			{
				...
				if (QuestionsanswersHelper::checkString($requiredField))
				{
					// change to false
	...
	public function save($data)
	{
		...
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				$table = JTable::getInstance('help_document', 'questionsanswersTable');

				if ($table->load(array('alias' => $data['alias'])) && ($table->id != $data['id'] || $data['id'] == 0))
				{
					$msg = JText::_('COM_QUESTIONSANSWERS_HELP_DOCUMENT_SAVE_WARNING');
				}
	...
	protected function _generateNewTitle($alias, $title = null)
	{
		...
		while ($table->load(array('alias' => $alias)))
		{
			...
			if (QuestionsanswersHelper::checkArray($title))
			{

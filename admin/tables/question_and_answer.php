...
class QuestionsanswersTableQuestion_and_answer extends JTable
{
	...
	{
		parent::__construct('#__questionsanswers_question_and_answer', 'id', $db);

		// Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_questionsanswers.question_and_answer'));
	}	
	...
	/**
	 * Overload the store method for the Question_and_answer table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New question_and_answer. A question_and_answer created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('question_and_answer', 'QuestionsanswersTable');

			if ($table->load(array('alias' => $this->alias, 'catid' => $this->catid)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_QUESTIONSANSWERS_QUESTION_AND_ANSWER_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_questionsanswers.question_and_answer.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_questionsanswers.question_and_answer.'.(int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = NULL, $id = NULL) 
	{
		...
		$asset->loadByName('com_questionsanswers');

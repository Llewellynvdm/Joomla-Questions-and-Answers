 ...
 **/
abstract class QuestionsanswersHelperRoute
{
	...
	*/
	public static function getQuestions_and_answersRoute($id = 0, $catid = 0)
	{
		if ($id > 0)
		{
			...
			$needles = array(
				'questions_and_answers'  => array((int) $id)
			);
			// Create the link
			$link = 'index.php?option=com_questionsanswers&view=questions_and_answers&id='. $id;
		}
		else
		{
			// Initialize the needel array.
		...
		if ($catid > 1)
		{
			$categories = JCategories::getInstance('questionsanswers.questions_and_answers');
	...
	*/
	public static function getCategoryRoute($id = 0)
	{
		if ($id > 0)
		{
			...
			// Create the link
			$link = 'index.php?option=com_questionsanswers&view=category&id='. $id;
		}
		else
		{
			// Initialize the needel array.
	...
	*/
	public static function getDownloadsRoute($id = 0, $catid = 0)
	{
		if ($id > 0)
		{
			// Initialize the needel array.
			$needles = array(
				'downloads'  => array((int) $id)
			);
			// Create the link
			$link = 'index.php?option=com_questionsanswers&view=downloads&id='. $id;
		}
		else
		{
			// Initialize the needel array.
		...
		if ($catid > 1)
		{
			$categories = JCategories::getInstance('questionsanswers.downloads');
			$category = $categories->get($catid);
			if ($category)
			{
				$needles['category'] = array_reverse($category->getPath());
	...
	/**
	 * Get the URL route for questionsanswers category from a category ID and language
	 *
	...
	public static function getCategoryRoute_keep_for_later($catid, $language = 0)
	{
		...
		{
			//Create the link
			$link = 'index.php?option=com_questionsanswers&view='.$view.'&category='.$category->slug;
	...
	protected static function _findItem($needles = null,$type = null)
	{
		...
		if (!isset(self::$lookup[$language]))
		{
			...
			$component  = JComponentHelper::getComponent('com_questionsanswers');
		...
		if ($active
			&& $active->component == 'com_questionsanswers'
